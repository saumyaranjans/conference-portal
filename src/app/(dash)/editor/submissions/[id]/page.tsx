...
export default async function EditorSubmissionPage({
  ...
  return (
    <>
      ...
      <PageHeader
        title={sub.title}
        subtitle={`${sub.tracks?.name ?? "No track"} · Version ${sub.version} · Submitted ${formatDate(sub.submitted_at)}`}
      ...
      <Section title="Editorial recommendation">
            ...
            {((decisions ?? []) as any[]).map((d) => (
              <div key={d.id} className="card card-pad">
                <div className="flex items-center gap-3">
                  ...
                  >
                    {d.is_final ? "Final (Editor-in-Chief)" : "Recommendation"}
                  </span>
                </div>
        ...
        {isFinal ? (
          ...
          <ActionForm action={recordRecommendation} className="card card-pad space-y-4">
            ...
            <div>
              <label className="label">Recommend to the Editor-in-Chief</label>
