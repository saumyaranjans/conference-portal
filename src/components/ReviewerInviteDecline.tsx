...
import { declineReviewerInvite } from "@/lib/actions";

/**
 * Landing page for the Decline link in a NEW-person reviewer invitation email.
 * Captures a brief reason, marks the invitation declined, notifies the Track
 * Editor, and thanks the invitee.
 */
export function ReviewerInviteDecline({
  token,
  valid,
  paperId,
  title,
  ...
  async function submit(e: React.FormEvent) {
    ...
    setState("busy");
    const res = await declineReviewerInvite(token, reason.trim());
        ...
        {state === "done" ? (
          ...
          <form onSubmit={submit} className="space-y-3 text-left">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 text-center">
              You are declining the invitation to review{" "}
